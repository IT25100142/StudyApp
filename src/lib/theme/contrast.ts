...
export function blendHexOpacity(foregroundHex: string, backgroundHex: string, foregroundOpacity: number): string {
  const fg = parseHex(foregroundHex)
  const bg = parseHex(backgroundHex)
  const a = Math.max(0, Math.min(1, foregroundOpacity))
  return toHex({
