...
test('creates subject inline and auto-starts timer when adding focus target', async ({ page }) => {
  ...
  const input = page.getByPlaceholder('What do you want to focus on?').first()
  await input.fill('Read chapter 1')
  await input.press('Enter')

  await expect(page.getByText('Read chapter 1').first()).toBeVisible({ timeout: 10000 })
...
test('auto-starts timer when selecting an existing focus target', async ({ page }) => {
  ...
  await input.press('Enter')
  await expect(page.getByText('First task').first()).toBeVisible({ timeout: 10000 })
  ...
  await page.getByRole('button', { name: /Select First task for timer|Deselect First task for timer/i }).click()
  await expect(page.getByRole('button', { name: /pause timer/i })).toBeVisible({ timeout: 5000 })
})
